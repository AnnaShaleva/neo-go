...
func (bc *Blockchain) init() error {
	...
	if err != nil {
		...
		if bc.config.EnableStateRoot {
			if err := bc.dao.InitMPT(0, bc.config.EnableMPTGC); err != nil {
				return err
			}
		}
	...
	if bc.config.EnableStateRoot {
		if err = bc.dao.InitMPT(bHeight, bc.config.EnableMPTGC); err != nil {
...
func (bc *Blockchain) GetStateProof(root util.Uint256, key []byte) ([][]byte, error) {
	...
	tr := mpt.NewTrie(mpt.NewHashNode(root), bc.config.EnableMPTGC, storage.NewMemCachedStore(bc.dao.Store))
